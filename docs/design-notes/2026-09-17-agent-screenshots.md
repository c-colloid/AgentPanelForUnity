...
**第三者の画像の扱い**: Grok の X 検索は、投稿に添付された写真(第三者の著作物)を
ツール結果として取得し、パネルがサムネイル表示する。公開する画像ではその写真の
領域だけを 24px ブロックのモザイクにする(`grok-x-search-with-request.png` と
`grok-x-search-panels.png` の依頼側パネル、(91,1050) から 480x270)。「X から画像を
取ってきた」ことは伝わり、中身は判別できない。ワールド名・作者名は公開情報の
引用なのでそのまま載せる(2026-09-17 ユーザー判断)。今後 X 検索の画面を撮るときも
同じ運用にする。
