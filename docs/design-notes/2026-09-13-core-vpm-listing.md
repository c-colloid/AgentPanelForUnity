...
### 7. 他エージェントの導入・認証の説明を作り直す(同日の追補)

README から「Codex / Gemini CLI / Grok Build の認証は操作ガイド 1 章」と
誘導していた先が、1 章末尾の引用ブロック 2 つ(環境変数名の列挙)で読み
にくかった。1 章に **1.1 「Claude 以外のエージェントを使う」** を新設し、
切り替えの 3 ステップ(エージェントを選ぶ → インストール → サインイン)と、
エージェントごとの「サインイン / API キー」の表、共通の注意(キーは
パネルに保存しない・環境変数はエディタ再起動・Claude Code 専用機能)に
分けた。README と操作ガイド冒頭のリンクは 1.1 節に向けた。

**Gemini CLI** は 2026-06-18 に個人向け(Google AI Pro / Ultra / 無料枠)
の提供が終了し、Google は Antigravity CLI への移行を案内している
(API キーと Code Assist Standard / Enterprise ライセンスでは引き続き
動く)。README の対応エージェント一覧からは外し、操作ガイドでは 1.1 節
末尾の注記(API キー / 企業ライセンス保持者向けにプリセットは残す、
新規には勧めない)に格下げした。パネルのプリセット自体(コード・UI
文言)は触っていない。Antigravity CLI が ACP に対応しているかは未確認
なので、対応エージェントとしては挙げていない。
