...
\* 印のモジュールは別売パッケージ `jp.colloid.agent-panel-pro` が必要です。未導入の間、該当トグルは設定画面で無効化され「Agent Panel Pro(未導入)で提供」と表示されます。
...
- **取り消せない操作**(アセット削除、プレハブへの Apply など)は、Claude が `confirm:true` を付けるまで実行されず、影響範囲(削除対象、他のインスタンス数)だけが返ります。実行前に会話で内容を確認してください。
- **時間のかかる操作**(ライトマップベイクなど)はジョブになり、Claude が `uap_job_status` で後から結果を取得します。Editor が処理中でも応答は返ります。
- **拡張プロファイル**(設定 > 拡張プロファイル): VRChat SDK3 / UniVRM / MagicaCloth2 / Final IK / Bakery を検出すると、その SDK の要点が Claude の指示に自動で追加されます(これら5つの同梱プロファイルは `jp.colloid.agent-panel-pro` が提供します)。プロジェクト独自のプロファイルは「確認して承認」で全文を確認してから有効になります。
...
| **Unity操作(UapOps)** | 有効化、モジュール(コア / プレハブ / エディタ / アニメーション / シーンビューのマーカー)、検証ゲート、サーバ状態(「ポート N で実行中」) |
...
[GitHub Issues](https://github.com/c-colloid/UnityAgentPanel/issues) に報告してください。
