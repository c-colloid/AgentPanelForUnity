...
**エージェントの切り替え**は設定 > エージェント、**接続先が提供するモデルの選択**はヘッダーのモデルピッカーで行います。利用できるモデルは CLI・認証方式・アカウントによって異なります。モデル一覧や切り替えの対応状況も接続先に依存します。詳しくは [モデルの選択](#10-モデルの選択) を参照してください。
...
| **エディタ** | Scene / Game ビューのスクリーンショット、任意のエディタメニューの実行、Play モードの開始・停止・一時停止・1 フレーム送り(`uap_play_mode`)、Unity コンソールのログ読み取り(`uap_console_logs`)とクリア(`uap_console_clear`)、Game View の解像度の取得・変更(`uap_game_view_size`。一覧に無いサイズは「Agent Panel 1920x1080」のような名前でドロップダウンに追加されます)。ライトマップベイク(非同期・プリフライト診断)と Bakery GPU Lightmapper 連携\*(Agent Panel Pro) |
...
- **動かして確かめる**: 「直したら実際に動かして確認して」と頼めば、エージェントが Play モードを開始し(`uap_play_mode`)、ゲームが出したログを読み(`uap_console_logs`)、終わったら停止します。Play モードの出入りはドメインリロードを伴うため、エージェント側は切り替わりを待ってから状態を確認する作りになっています。コンパイル中は実行を断ります。Player Settings の Run In Background が OFF の場合、Play 中にエディタからフォーカスを外すと Unity 操作がタイムアウトすることがあります(開始時に警告が出ます)。
...
- **uLoop 連携**(設定 > uLoop連携): [uLoopMCP](https://github.com/hatayama/uLoopMCP) を導入しているプロジェクトでは、「uloopコマンドを許可」で権限ルールを、「指示スニペットを挿入」でカスタム指示を追加できます。未導入なら「uLoopを導入」から導入できます(変更内容を確認してから実行)。
  - **導入すると常駐するもの**(カード先頭の注記。2026-09-21 に uLoop 3.6.3 で計測): エディタ側サーバ(エディタ起動のたびに自動で立つ)、16ms ごとのティックポンプ(非フォーカスでアイドルでもエディタを回し続けるので、裏に回しても休まなくなります)、エージェント用スキル約 21 件の説明文(約 5.7KB ≒ 1.5〜2k トークン。uloop を呼ばないセッションにも毎回載ります)。**この 3 つはこのパネルからは止められません**(uLoop 側の持ち物です)。
  - **弱めたいとき**: `Window > Unity CLI Loop > Server` はサーバを今のエディタセッションだけ停止します(ティックポンプは止まらず、次回起動でサーバも復活します)。`Window > Unity CLI Loop > Settings` でツールを個別に無効化すると、そのツールのスキルも配置対象から外れてトークン分も減ります。完全に止める唯一の方法は Package Manager でパッケージを削除することです。
  - **エージェントに uloop を使わせない**(コスト行の直下のトグル。既定はオン): オフにすると、(1) エージェントへの指示文に「uloop は拒否される」と明記し、(2) 次の接続の起動引数に各シェル用の拒否パターンを足し、(3) それでも届いた uloop コマンドを拒否して、代わりに使う `uap_*` ツールを会話に表示します。反映は次の接続からです(許可/禁止ツールリストと同じ)。**これはエージェントの利用を止めるだけで、uLoop 自体は止まりません**(上の「導入すると常駐するもの」はそのまま残ります)。
  - **削除**(導入済みのとき「uLoop を削除」ボタン): 常駐を本当に止められるのはパッケージを外すことだけなので、導入ボタンと対称の削除導線を用意しています。導入時と同じく **変更内容(manifest.json の差分)を先に表示** し、`.uap-backup` を取ってから書き込みます。依存の削除は Unity が行い、このパネルは **自分が追加した OpenUPM の設定だけ** を片付けます(自分が書いていない広いスコープ・他のパッケージがまだ使っているスコープ・OpenUPM 以外のレジストリには触れません)。片付けは削除が終わってから走る 2 段目で、ここが失敗しても削除自体は成功のまま「manifest.json に設定が残っています」と表示します。設定側の `uloop` 許可行や指示スニペットは残ります(確認カードにも明記)。設計: `docs/design-notes/2026-09-22-uloop-remove-from-panel.md`。
  - Play モード操作・コンソールのログ読み取りとクリア・スクリーンショット・スクリプトのコミット・テスト実行は、このパネル自身の `uap_*` ツールで賄えます(uLoop なしでも困りません)。詳細と計測値: `docs/design-notes/2026-09-21-uloop-always-loaded-cost.md`。
...
| **Unity 連携 > uLoop連携** | 常駐コストの注記、エージェントに uloop コマンドを使わせる、uLoop を導入 / uLoop を削除、uloop コマンドを許可、指示スニペットを挿入 |
