...
| **Agent Panel Pro の更新** | 購入時のレジストリ URL と製品キー。「キーを保存」で `~/.upmconfig.toml`(または `UPM_USER_CONFIG_DIR`)にトークンを、`Packages/manifest.json` に scoped registry を書き込み、以後 Package Manager の My Registries から Pro を導入・更新できる。パネルはキーを保存しない |
