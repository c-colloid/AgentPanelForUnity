...
- **ユーザーからのスケッチ(線)**: コンテキストバーの「スケッチ」メニュー(または Scene ビューのツールバーの「Agent Sketch」トグル)で描画モードに入り、Scene ビューを左ドラッグすると線(S1, S2, ...)が描けて、点列としてエージェントに渡せます(エージェントは `uap_stroke_list` で全点を読めます)。**Esc** でモードを抜けます。
  - **平面に描く**: 先に深度を決めた平面に描きます。平面が周囲のオブジェクトを切る位置に水色の**輪郭線**が出るので、どの奥行きに平面があるかが分かります。**ホイール / `[` `]`** で深度を前後、**F** でカーソル下の面に合わせ、**X / Y / Z** でワールド軸の平面に固定、**C** でカメラ正対に戻します。**Shift** を押しながらドラッグすると直線になります。
  - **メッシュ表面に描く**: カーソル下のメッシュ表面に沿って描きます。描いたオブジェクトと各点の法線も一緒に伝わります。
  - Pro のメッシュツールはこの線をそのまま使えます: 線に沿って溝を掘る / 盛り上げる(`uap_mesh_edit`)、線に沿ったチューブや、閉じた線の外形で穴をあける・板を貼る(`uap_mesh_create` の `sdf` / `extrude`)。
- 「Scene ビューのマーカーとピンをすべて消す」でまとめて消せます(スケッチも含みます)。
