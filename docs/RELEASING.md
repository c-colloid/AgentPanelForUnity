...
- v0.60.0-beta.1: 設定の概要フッターに CLI 更新確認(npm レジストリ)と `claude update` の実行ボタン、Pro バージョンのピルを追加
- v0.60.0-beta.2: CLI 更新後に自動で再接続(待機中は即時・応答中は完了後)+「今すぐ再接続」ボタン、バージョン表記が旧版のまま残る問題と再接続案内が消えない問題を修正
