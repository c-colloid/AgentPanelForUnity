...
| v0.54.0 | Pro の新モジュール `mesh`(`uap_mesh_create` / `uap_mesh_inspect` / `uap_mesh_edit` / `uap_mesh_boolean`: コンパイル不要のメッシュ生成・調査・範囲編集・ブーリアン。`docs/design-notes/2026-09-15-mesh-create.md`、`2026-09-15-mesh-edit-sdf-boolean.md`)のための設定行「メッシュ生成」を 設定 > Unity操作(UapOps) に追加(既定 OFF、Core 単体ではトグルが無効化され「別売の Agent Panel Pro 拡張パッケージが必要です(未導入)」のヒントが出る)。Core 自体の挙動は不変(2026-09-15) |
