...
namespace Colloid.AgentPanel.UI
{
    ...
    public sealed class SettingsView : IAgentPanelView
    {
        ...
        private Button _cliUpdateReconnectButton;
        ...
        private void BuildOverviewFooter(VisualElement parent)
        {
            ...
            // The one action the post-update notice asks for, next to it
            // (the panel also does it on its own once the turn ends).
            _cliUpdateReconnectButton = new Button(OnReconnectClicked) { text = L10n.S.SettingsReconnectNowButton };
            _cliUpdateReconnectButton.AddToClassList("uap-settings-link-btn");
            _cliUpdateRow.Add(_cliUpdateReconnectButton);
        ...
        private void RefreshCliVersionLabel()
        {
            ...
            // A live connection was spawned from the probed path, so the
            // probe's number IS the running CLI's -- unless an update has
            // replaced the binary since and the reconnect is still pending.
            // (After `claude update` + reconnect this is the normal case: the
            // resumed connection re-emits system/init only after its first
            // message.)
            bool unconfirmed = fromBinaryProbeOnly
                && (!connectedOrConnecting || AgentHub.CliUpdateReconnectPending);
            _cliVersionLabel.text = string.IsNullOrEmpty(cliVersion)
                ...
                : L10n.F(unconfirmed ? L10n.S.SettingsCliVersionUnconfirmedFmt : L10n.S.SettingsCliVersionFmt,
                    cliVersion);
        ...
        private void RefreshCliUpdateState()
        {
            ...
            // Nothing more to install while the finished update waits for
            // its reconnect (the probe may not have re-read the binary yet).
            bool canUpdate = !string.IsNullOrEmpty(_cliResolvedPath) && !AgentHub.CliInstallRunning
                && !AgentHub.CliUpdateReconnectPending;
            ...
            bool reconnectPending = AgentHub.CliUpdateReconnectPending;
            _cliUpdateReconnectButton.style.display = claude && reconnectPending && !updating
                ? DisplayStyle.Flex : DisplayStyle.None;
            string text = claude
                ? DescribeCliUpdateState(checking, AgentHub.CliUpdateChecked, AgentHub.LatestCliVersion,
                    _cliInstalledVersionForUpdate, updating, AgentHub.LastCliUpdateResult, reconnectPending,
        ...
        /// running or failed update outranks the check result it came from;
        /// a successful one only while its reconnect is still pending --
        /// after that the check result (now against the new binary) is the
        /// news, so the "reconnect" notice cannot linger.
        /// </summary>
        internal static string DescribeCliUpdateState(bool checking, bool checkedOnce, string latest,
            string installed, bool updating, CliInstallResult lastUpdate, bool reconnectPending,
            long startedUtcTicks, long nowUtcTicks)
        {
            ...
            if (lastUpdate != null && lastUpdate.Success && reconnectPending)
            {
                return L10n.F(L10n.S.SettingsCliUpdateDoneFmt, lastUpdate.LastLine ?? string.Empty);
            }
            if (lastUpdate != null && !lastUpdate.Success)
            {
