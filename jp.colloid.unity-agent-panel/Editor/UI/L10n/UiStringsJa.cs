    ...
    internal static class UiStringsJa
    {
        ...
        public static UiStrings Create()
        {
            return new UiStrings(
                ...
                firstRunLoginLead: "Claude Code CLI はインストール済みですが、サインインしていません。「ログイン」を押すと CLI のサインインフローが始まります。設定の「エージェント」カードが開き、ブラウザ用リンクと確認コードの入力欄が表示され、完了するとパネルは自動的に再接続します。",
                ...
                hubAutoContinuePendingWillContinue:
                    ...
                    + "ドメインリロードします。完了後、パネルがコンパイル結果を添えた継続ターンを"
                    + "自動送信します。",
                ...
                settingsAutoContinueInterruptedTooltip:
                    "エージェント起因ではないドメインリロード(スクリプトを保存してエディタに戻った、手動で再コンパイルした、Playモードに入った等)で実行中のターンが中断された場合、パネルはセッションを再接続したうえで、バナーの [続きを実行] ボタンと同じメッセージをクリックを待たずに送信します。回数の上限はなく、CLI の連続終了で接続が停止しているときと、ユーザーが停止したときだけ止まります。送信は必ず会話ログに残ります。",
                ...
                settingsSectionDiagnostics: "CLI の出力(stderr)",
                ...
                ctxSketchButton: "スケッチ",
                ctxSketchButtonArmedPlane: "平面に描画中...",
                ctxSketchButtonArmedSurface: "表面に描画中...",
                ctxSketchTooltip: "Scene ビューに線を描いて点列としてエージェントに渡します。平面モードは決めた深度の平面に描き(ホイール / [ ] で深度、F でカーソル下の面に合わせる、X/Y/Z で軸平面に固定、C でカメラ正対、Shift で直線)、表面モードはカーソル下のメッシュ表面に描きます。Esc で終了。",
                ctxSketchMenuPlane: "平面に描く(深度を決める)",
                ctxSketchMenuSurface: "メッシュ表面に描く",
                ctxSketchMenuStop: "スケッチをやめる",
                ctxSketchPlaneToolbarLabel: "平面",
                ctxSketchPlaneTooltip: "エージェント スケッチ: 決めた深度の平面に描く(ホイール / [ ]: 深度、F: 面に合わせる、X/Y/Z/C: 平面、Esc: 終了)",
                ctxSketchSurfaceToolbarLabel: "表面",
                ctxSketchSurfaceTooltip: "エージェント スケッチ: カーソル下のメッシュ表面に描く(Esc: 終了)",
                ctxSketchChipLabelFmt: "スケッチ S{0}",
                ctxSketchChipTitleFmt: "シーンスケッチ S{0}",
                ctxSketchDepthReadoutFmt: "スケッチ平面: 深度 {0} m、{1}",
                ctxSketchAxisCamera: "カメラ正対",
                ctxSketchContourPartial: "(輪郭は一部のみ: シーンが大きすぎます)",
                ctxSketchSurfaceBehindFmt: "カーソル下の面: 平面の {0} m 後ろ",
                ctxSketchSurfaceInFrontFmt: "カーソル下の面: 平面の {0} m 手前",
                ctxSketchKeyHints: "ホイール / [ ]: 深度   F: 面に合わせる   X Y Z / C: 平面   Shift: 直線   Esc: 終了",
                ctxStrokePayloadHeaderFmt: "Scene sketch S{0} (user-drawn stroke in the Scene view)",
                ctxStrokePayloadModeFmt: "mode: {0}",
                ctxStrokePayloadStatsFmt: "points: {0}, length: {1} m, closed: {2}",
                ctxStrokePayloadClosedYes: "yes",
                ctxStrokePayloadClosedNo: "no",
                ctxStrokePayloadPlaneFmt: "plane: origin {0}, normal {1}",
                ctxStrokePayloadObjectsFmt: "on objects: {0}",
                ctxStrokePayloadBoundsFmt: "bounds: {0} - {1}",
                ctxStrokePayloadPointsFmt: "points: {0}",
                ctxStrokePayloadMorePointsFmt: "({0} of {1} points shown; uap_stroke_list id={2} returns all)",
                ctxStrokePayloadAllPointsFmt: "(all points shown; uap_stroke_list id={0} returns them with normals)",
                ...
                hubAcpCommandNotFoundErrorFmt: "{0} のコマンド '{1}' が見つかりません。確認したパス: {2}。設定 > エージェント > 詳細設定 でコマンドを指定してください。",
                ...
                settingsAcpAuthMethodHint: "ACP の認証メソッド ID。空欄ならエージェントが最初に提示するサインイン方法を使います。",
                ...
                settingsAcpLimitationsTooltip: "ACP エージェントでは: スクリプトゲートのフックは使えません(Claude Code 専用)。カスタム指示とサブエージェントのモデル設定は、新しいセッションの最初に指示文として送られます。履歴にはパネル自身が保存した会話の写しが並び、セッションを再開できないエージェントは新しいセッションを開始して次のメッセージと一緒に会話ログを受け取ります。サインイン(設定 > エージェント)は、ログインコマンドを持つエージェントではそのコマンドをパネル内で実行します。権限カード・思考ブロック・Unity 操作ツールは同じように動作します。",
                ...
                hubAcpSignInRequiredNoteFmt: "{0} にサインインしていないため、再接続を繰り返しませんでした。設定 > エージェントからサインインする(またはターミナルで `{1}` を実行する)か、そのあと「再接続」を押してください。",
                hubAcpHandshakeDeathNoteFmt: "{0} が接続確立前に終了したため({1})、自動では再試行しませんでした。インストールとサインイン(`{2}`)を確認してから、設定 > エージェントの「再接続」を押してください。",
                ...
                settingsClaudeAuthTooltip: "Claude Code 専用(ACPエージェントには影響しません)。「自動」(推奨・デフォルト)は"
                    ...
                    + "ログインが使われます。",
                settingsClaudeAuthOptionAuto: "自動(CLI に任せる)",
                ...
                firstRunAcpLoginLeadFmt: "{0} はインストールされていますが、サインインしていないと報告しました。「サインイン」を押すとパネル内で `{1}` を実行します。設定の「エージェント」カードにブラウザ用のリンクが表示され、コマンドが終了するとパネルが自動で再接続します。",
                ...
                settingsBackendHint: "次回の再接続時に適用されます。以下はすべてこのエージェントについての表示と設定です。",
                ...
                settingsTabOverview: "概要",
                settingsTabShortOverview: "概要",
                settingsTabShortAgent: "エージェント",
                settingsTabShortPanel: "パネル",
                settingsTabShortUnity: "Unity",
                settingsTabShortConnection: "接続",
                settingsOverviewSetupTitle: "セットアップ",
                settingsOverviewEffectiveTitle: "いま効いている設定",
                settingsOverviewAgentLabel: "エージェント",
                settingsOverviewSignInLabel: "サインイン",
                settingsOverviewUnityOpsLabel: "Unity 操作",
                settingsOverviewDangerLabel: "権限確認",
                settingsOverviewPermissionLabel: "権限モード",
                settingsOverviewAutoApproveLabel: "自動承認",
                settingsOverviewModelLabel: "既定モデル",
                settingsOverviewLanguageLabel: "言語",
                settingsOverviewChangeButton: "変更",
                settingsOverviewSetUpButton: "設定する",
                settingsOverviewSignInButton: "サインイン",
                settingsOverviewEnableButton: "有効にする",
                settingsOverviewUnityOpsOffDetail: "エージェントがエディタを直接操作できるようになります。",
                settingsOverviewAgentNotFound: "見つかりません",
                settingsOverviewDangerArmed: "すべての権限確認をスキップ中",
                ...
                settingsWebSearchProviderLabel: "検索プロバイダ",
                settingsWebSearchProviderHint: "自前の Web 検索を持たないエージェント向けの uap_web_search。Brave Search API か Tavily を自分のキーで使います。",
                settingsWebSearchApiKeyLabel: "検索 API キー",
                settingsWebSearchApiKeyHint: "このプロジェクトの UserSettings に保存されます。空ならツールは設定場所をエージェントに伝えます。",
                settingsSearchPlaceholder: "設定を検索",
                settingsSearchClearButton: "クリア",
                settingsSearchNoMatch: "一致する設定がありません。ラベルやヒントの語で探してください。",
                settingsPillOff: "オフ",
                settingsPillPortFmt: "ポート {0}",
                settingsPillSignedIn: "サインイン済み",
                settingsPillNotSignedIn: "未サインイン",
                settingsPillSigningIn: "サインイン中...",
                settingsPillConnecting: "接続中...",
                settingsPillChecking: "確認中...",
                settingsPillNotSet: "未設定",
                settingsPillLinesFmt: "{0} 行",
                settingsPillItemsFmt: "{0} 件",
                settingsPillIgnoredFmt: "{0} 件を無視",
                settingsPillDetectedFmt: "{0} 件検出",
                settingsPillNoneDetected: "検出なし",
                settingsPillInstalled: "導入済み",
                settingsPillDisabled: "無効",
                ...
                settingsCliUpdateDoneFmt: "更新が完了しました: {0} 新しいバージョンへ自動で再接続します(応答中の場合は応答の完了後)。",
