...
コーディングエージェントを Unity エディタの中で使うためのチャットパネルです。Claude Code を中心に、Codex・Gemini CLI・Grok Build などの ACP 対応 CLI にも切り替えられます。エディタにドッキングしたパネルから、コードを書く・ファイルを直す・シーンを操作するといった作業をエージェントに頼めます。ツールを実行する前には許可カードが出て、スクリプトの再コンパイルをまたいでも会話は続きます。
...
- **Claude 以外のエージェント** — Codex、Gemini CLI、Grok Build など ACP 対応の CLI にも切り替えられます。
...
Codex / Gemini CLI / Grok Build を使う場合の認証は [操作ガイド 1 章](docs/USER-GUIDE.md#1-パネルを開く初回セットアップ) にまとめています。
